...
export default function SearchHeader({ filtersState }: { filtersState: FiltersStateType }) {
  const { filters, setSortOption } = filtersState;

  const tabs = [
    { label: '인기순', sort: 'productId', direction: 'DESC' },
    { label: '신상품순', sort: 'createdAt', direction: 'DESC' },
    { label: '낮은 가격순', sort: 'price', direction: 'ASC' },
    { label: '높은 가격순', sort: 'price', direction: 'DESC' },
  ];
          ...
          {tabs.map((tab, idx) => {
            ...
            return (
              <div key={tab.label} className="flex items-center gap-4">
                <button
                  type="button"
                  onClick={() => setSortOption(tab.sort, tab.direction)}
                  className={
                    isActive
